...
class _RatesDetailPageState extends State<RatesDetailPage> {
  ...
  void initState() {
    ...
    // Auto-scroll to initial chip after build
  ...
  void _scrollToChip(int index) {
    ...
    // Calculate approximate position (chip width + padding)
    const chipWidth = 120.0; // Approximate width of each chip
    const padding = 12.0; // Right padding between chips
    final targetPosition = index * (chipWidth + padding);

    // Get the viewport width
    final viewportWidth = MediaQuery.of(context).size.width;

    // Calculate scroll offset to center the selected chip
    final centerOffset = targetPosition - (viewportWidth / 2) + (chipWidth / 2);

    // Clamp between min and max scroll extent
    final maxScroll = _chipScrollController.position.maxScrollExtent;
    final scrollTo = centerOffset.clamp(0.0, maxScroll);
